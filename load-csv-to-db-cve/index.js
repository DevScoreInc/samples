...
async function main() {
    ...
    let loadCSVFile = async (filePath) => {
        return new Promise(async (resolve, reject) => { 
            ...
            let rows = [];
            // replace "/" in CSV file header with "--" 
    ...
    let getDataForDate = async (varName, cveDate, documentId) => {
        let ret = await _context.dbLib.getTableRowsWithFilterAndSort(varName, "Notes--Note--1--__text", "==", cveDate, false, 100, documentId);
        return ret;
    }
    ...
    let cveDataforDate = await getDataForDate(varName, cveDate, documentId);
